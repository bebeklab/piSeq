...
#unique <- read.table("../alignments/s", header = FALSE, stringsAsFactors = FALSE)
...
colnames(tRNA)[2] <- "tRNA Name"
rmsk_plus40_tRNA <- rmsk_plus40[grepl(pattern = "tRNA", rmsk_plus40$'Extended Repeat Name'),]
colnames(rmsk_plus40_tRNA)[2] <- "Extended tRNA Name"
...
voom_top_sRNA$'Extended tRNA' <- ifelse(voom_top_sRNA$sRNA %in% rmsk_plus40_tRNA$sRNA, T, F)
voom_top_sRNA <- merge(voom_top_sRNA, rmsk_plus40_tRNA[,c(1:2)], by = 'sRNA', all.x = T)
...
voom_top_sRNA$'Extended Repeat Masker' <- ifelse(voom_top_sRNA$sRNA %in% rmsk_plus40$sRNA, T, F)
voom_top_sRNA <- merge(voom_top_sRNA, rmsk_plus40[,c(1:2)], by = 'sRNA', all.x = T)
...
voom_sRNA$'Extended tRNA' <- ifelse(voom_sRNA$sRNA %in% rmsk_plus40_tRNA$V1, T, F)
