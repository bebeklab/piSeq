...
clusterStart <- function(cluster_coordinates){
  return(as.numeric(unlist(strsplit(cluster_coordinates, split = "-"))[1]))
}
clusterEnd <- function(cluster_coordinates){
...
files <- list.files(protrac_folder, full.names = TRUE, recursive = TRUE, pattern = "clusters.fasta")

#load the proTRAC cluster
loadClusters <- function(file){
  
  clusters <- read.table(file, sep = "\n", stringsAsFactors = FALSE)
...
for(row in 1:nrow(stem_diff_clusters)){
  ...
  for(n in row:nrow(stem_diff_clusters)){
    ...
    if(stem_diff_clusters$chromosome[row] == stem_diff_clusters$chromosome[n]){
      #if they overlap
      if(abs(stem_diff_clusters$start[row] - stem_diff_clusters$start[n]) <= 1000 | abs(stem_diff_clusters$end[row] - stem_diff_clusters$end[n]) <= 1000){
